...
The sweep is cheap (one in-memory pass), unconditional, and idempotent. It is the single canonical place that applies cooldown expiry, which keeps the pickup filter simple.
...
**Per-record isolation.** One bad record does not kill the batch. The driver wraps each record in its own try/except. It logs and counts the error, then continues the cycle.

**Fast-path normalisation.** When a record already carries contact email and phone, the pipeline skips the waterfall lookup entirely. No provider credits consumed.
...
The mix is intentional. `L002/L003/L007/L008` advance on a clean mobile + email hit, and `L006` advances via the fast-path. Three records park at increasing waterfall depth. `L001` found a phone but no email at provider A, `L005` fell through to provider B, and `L004` fell all the way through to provider C. One messy row has no owner name, so the pipeline skips it entirely.
...
**Score weights are illustrative, not tuned.** The weight constants in `scoring.py` demonstrate the signal architecture. I've tuned every production scoring system I've built against real outcome data. Treat the illustrative weights here as starting points only.
